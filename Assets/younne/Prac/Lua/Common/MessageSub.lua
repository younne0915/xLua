...
function TEMP.GenMessageSub(name)
	...
	function Sub.Register(first, second, func, self)
		if type(first) ~= "number" or type(second) ~= "number" then
			GameLog.LogError("message sub '" .. Sub.name .. "' register failed: first, second param must be integer->%s, %s", tostring(first), tostring(second));
			return -1;
		end
		...
		if list.Count() > 200 then
			GameLog.LogWarning("message sub func register too much->%d, %d, %d", first, second, list.Count());
		end
	...
	function Sub.SendMessage(first, second, ...)
		if type(first) ~= "number" or type(second) ~= "number" then
			if GameLog then
				GameLog.LogError("message sub '" .. Sub.name .. "' send failed: first, second param must be integer->%s, %s", tostring(first), tostring(second));
			end
			return;
		end
		...
		for i = 1, count, 1 do
			...
			if t ~= nil then
				if t._self then
					--if not GameUtils.TryCatch(t._func, t._self, data) then
					...
					--if not GameUtils.TryCatch(t._func, data) then
	...
	return GameUtils.ShallowConst(Sub);
end


--MessageSub = GameUtils.DeepConst(MessageSub);
